<x-layout>
    <x-slot name="content">
        <main class="max-w-lg mx-auto">
            @if ($project)
                <h1 class="text-center font-bold text-xl mb-3">Edit: {{ $project->title }}</h1>
            ...
            <div class="mb-6">
                ...
                <textarea name="excerpt" id="excerpt" value="{{ old('excerpt') ?? $project?->excerpt }}"
                    class="border border-gray-400 rounded p2 w-full" required> {{ old('excerpt') ?? $project?->excerpt }}</textarea>
                @error('excerpt')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="body" id="body" class="border border-gray-400 rounded p2 w-full" required> {{ old('body') ?? $project?->body }}</textarea>
                @error('body')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="category_id" id="category_id">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @if ($category->id == old('category_id') || $project?->category_id == $category->id) selected @endif>
